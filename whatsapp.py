...
try:
    ...
    print(
        "Beautiful Soup Library is required to make this library work(For getting participants list for the specified group).\npip3 install beautifulsoup4"
    )
...
class WhatsApp:
    ...
    def participants_count_for_group(self, group_name):
        ...
        try:
            click_menu = WebDriverWait(self.browser, self.timeout).until(
        ...
        while True:
            try:
                participants_count = self.browser.find_element_by_css_selector(
    ...
    def get_group_participants(self, group_name):
        ...
        for v in range(1, 70):
            ...
            self.browser.execute_script(
                "arguments[0].scrollTop = " + str(v * 300), scrollbar
            )
            time.sleep(0.10)
        ...
        try:
            ...
            send_msg = WebDriverWait(self.browser, self.timeout).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        "/html/body/div/div/div/div[4]/div/footer/div[1]/div[2]/div/div[2]",
                    )
                )
            )
            messages = message.split("\n")
    ...
    def send_picture(self, name, picture_location, caption=None):
        ...
        try:
            ...
            attach_type_xpath = (
                "/html/body/div[1]/div/div/div[4]/div/footer/div[1]/div[1]/div[2]/div/span/div/div/"
            ...
            attach_img_btn.send_keys(
                picture_location
            )  # get current script path + img_path
            time.sleep(1)
            if caption:
                caption_xpath = (
                    "/html/body/div[1]/div/div/div[2]/div[2]/span/div/span/div/div/div[2]/div[1]/span/"
    ...
    def send_document(self, name, document_location):
        ...
        try:
            ...
            send_file_xpath = "/html/body/div[1]/div/div/div[2]/div[2]/span/div/span/div/div/div[2]/span/div/div/span"
            attach_type_xpath = (
                "/html/body/div[1]/div/div/div[4]/div/header/div[3]/div/div[2]/span/div/div/ul/li[3]"
                "/button/input"
            )
            # open attach menu
    ...
    def clear_chat(self, name):
        ...
        WebDriverWait(self.browser, self.timeout).until(
            EC.presence_of_element_located((By.XPATH, clear_xpath))
        )
    ...
    def get_profile_pic(self, name):
        ...
        try:
            open_profile = WebDriverWait(self.browser, self.timeout).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
            ...
            print("nothing found")
        try:
            open_pic = WebDriverWait(self.browser, self.timeout).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
            ...
            print("Nothing found")
        try:
            img = WebDriverWait(self.browser, self.timeout).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
    ...
    def create_group(self, group_name, members):
        ...
        next_step = self.browser.find_element_by_css_selector(
            "._3hV1n > span:nth-child(1)"
        )
        next_step.click()
        group_text = self.browser.find_element_by_css_selector(
            ".bsmJe > div:nth-child(2)"
        )
    ...
    def set_group_picture(self, group_name, picture_location):
        ...
        search.send_keys(
            group_name + Keys.ENTER
        )  # we will send the group name to the input key box
        try:
            menu_xpath = (
                "/html/body/div[1]/div/div/div[4]/div/header/div[3]/div/div[3]/div/span"
            )
    ...
    def get_invite_link_for_group(self, groupname):
        ...
        try:
            ...
            invite_link.click()
            WebDriverWait(self.browser, self.timeout).until(
                EC.presence_of_element_located((By.ID, "group-invite-link-anchor"))
            )
    ...
    def exit_group(self, group_name):
        ...
        confirm_exit = self.browser.find_element_by_css_selector(
            "div._1WZqU:nth-child(2)"
        )
        confirm_exit.click()
    ...
    def send_anon_message(self, phone, text):
        ...
        send_message.click()
        confirm = WebDriverWait(self.browser, self.timeout + 5).until(
            EC.presence_of_element_located(
                (
                    By.XPATH,
                    "/html/body/div/div/div/div[4]/div/footer/div[1]/div[2]/div/div[2]",
    ...
    def is_message_present(self, username, message):
        ...
        search_bar = WebDriverWait(self.browser, self.timeout).until(
            EC.presence_of_element_located(
                (
                    By.CSS_SELECTOR,
                    "._1i0-u > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > span:nth-child(1)",
                )
            )
        )
        search_bar.click()
    ...
    def get_starred_messages(self, delay=10):
        starred_messages = []
        self.browser.find_element_by_css_selector(
            "div.rAUz7:nth-child(3) > div:nth-child(1) > span:nth-child(1)"
        ).click()
        chains = ActionChains(self.browser)
        time.sleep(2)
        for i in range(4):
        ...
        for message in messages:
            try:
                ...
                message_text = soup.find(
                    "span", class_="selectable-text invisible-space copyable-text"
                ).text
                message.click()
